...
mutationdata = mutationdata %>% rename(mutationCoverage = coverage, 
                                       conditionCoverage = elementsCovered, 
...
```{r echo=FALSE}
boxplot(mutationdata$mutationCoverage, mutationdata$conditionCoverage, ylab="Code coverage", 
        names = c("Mutation coverage", "Condition coverage"), main="Code coverage distributions")
```
...
```{r echo=FALSE}
wilcox.test(solved$conditionCoverage, unsolved$conditionCoverage, alternative = 'g')
```

**Linear mixed effects model**

```{r echo=FALSE}
branch.fit.correctness = lmer(projectScore ~ conditionCoverage + (1 | userId), data = mutationdata)
